...
export function ScoutSection() {
  ...
  useEffect(() => {
    fetchScoutDashboard().then(setData).catch(() => {});
  }, []);
        ...
        <div className="mb-3 grid grid-cols-2 gap-6">
          <div>
            ...
            <div className="mt-1.5 flex items-baseline gap-2">
              <span className="text-3xl font-bold" style={{ color: accent, ...num }}>{data.credits.balance}</span>
            ...
            {open && (
              <>
                <p className="mt-2 text-sm text-gray-400">
                  補充: {formatReplenishDate(data.credits.nextReplenishDate)}（+{Math.min(data.credits.monthlyAllowance, data.credits.maxStock - data.credits.balance)}通 / 上限{data.credits.maxStock}通）
                </p>
                <div className="mt-4 border-t border-gray-100 pt-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span className="font-medium text-gray-700"><span className="text-lg font-bold" style={num}>{data.replyRate}</span> %</span>
                    </div>
                  </div>
                </div>
              </>
          ...
          <div>
            ...
            <div className="mt-1.5 flex items-baseline gap-2">
              <span className="text-3xl font-bold text-gray-900" style={num}>{data.pending.total}</span>
                  ...
                  {data.pending.byMonth.map((m) => (
                    <div key={m.month} className="flex items-center justify-between text-sm">
                      ...
                      <span className="flex items-baseline">
                        <span className="inline-flex items-baseline justify-end font-medium text-gray-700" style={{ width: "3.5rem" }}><span className="text-lg font-bold" style={num}>{m.count}</span><span className="ml-1">通</span></span>
            ...
            <div className="mt-2 flex items-center justify-between text-sm">
              ...
              <span className="font-medium text-gray-700"><span className="text-lg font-bold" style={num}>{data.avgReplyDays}</span> 日</span>
            </div>
            {open && (
              <Link
