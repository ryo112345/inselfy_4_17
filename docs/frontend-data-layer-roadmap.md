...
**⚠️ 横展開の前提条件:** mutator の refresh は候補者用固定で、企業セッションの silent refresh が
SDK 経路に存在しない。companyFetch ベースのページを移行する前に
**`docs/auth-refresh-hardening.md` の修正1・2を先に入れること**（rotation の per-token 化＋
mutator の領域別 refresh）。
