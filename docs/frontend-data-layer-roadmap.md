...
**残作業（Phase 3 のスコープ外に切り出し）:** companyFetch ベースのページ
（company/teams・company/profile・CompanyHeader・useCompanyProfile・company/jobs/[jobId]）は
生 fetch のまま。auth-refresh-hardening 修正2で SDK 経路の企業 refresh が入ったため
orval 移行可能になった。修正3（auth 系一本化）と合わせて別途実施する。
