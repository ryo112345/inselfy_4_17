...
**3-2 横展開（2026-07-17 完了）— hey-api は完全撤去済み:**
- **生成フック直用に移行:** scout（一覧・詳細・テンプレ CRUD・設定・一括操作を
  useQuery/useMutation 化、api.ts 削除）
- **api.ts 内部だけ orval 平関数化（シグネチャ不変）:** messaging / interview /
  job-posting / timeline / search / articles / work-values / career-interest /
  integrated-report / company-profile / profile（fetchPanelData 含む）。
  F 分類（WS 駆動・命令的フロー・加工あり）のため React Query 化はせず内部swap。
  404→null 等の加工は ApiError の try/catch に置換
- **単発ページ:** setup / signup / company register / diagnose を orval 直呼びに
- **SSR cookie 転送:** profile / work_values / career_interest / integrated-report の
  4ページを `orval/server` に一本化（hey-api server.ts は削除）
- **hey-api 撤去:** `external/client/api/generated`・`client.ts`・`server.ts` 削除
  （▲17,641行）、generate-ts.sh から hey-api 生成を除去、drift 検査・biome 除外・
  `@hey-api/openapi-ts` 依存も撤去。orval 再生成は差分ゼロ
- 検証: tsc / biome クリーン、e2e 19本全 PASS、scout 4ページの実ブラウザスモーク、
  /jobs・/profile の SSR レンダリング確認
- `orval-check` 検証ページ／スペックは refresh 経路の e2e として存続（削除しない）

**残作業（2026-07-17 完了）:** companyFetch ベースのページ
（company/teams 一覧・new・[id]、company/profile 表示・edit・preview、CompanyHeader、
useCompanyProfile、company/jobs/[jobId]）を orval 平関数の直呼びに移行。同時に
auth-refresh-hardening 修正3（auth-context / company-auth-context / viewer.ts の
生 fetch 一本化）を実施し、companyFetch と context 内の手書き refresh フローを撤去した。
これでフロントの HTTP 経路は orval mutator（+ SSR は viewer.ts の明示 Cookie）に一本化。
