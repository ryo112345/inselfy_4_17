...
| # | 状態 | 項目 | 規模 | 種別 |
|---|------|------|------|------|
| F21 | [ ] | AIレポートセクション3実装の統合（useTypewriter・ポーリング込み） | 中 | 重複排除 |
| F22 | [ ] | `isOwner = true` デフォルトの廃止 | 小 | 安全性 |
| F23 | [ ] | サーバー用 SDK クライアントで cookie 転送を自動化 | 中 | 統一 |
| F24 | [ ] | 401 インターセプタの /login リダイレクトをオプトアウト可能に | 小 | 安全性 |
| F25 | [ ] | レポート存在チェックの軽量化（本文の二重取得解消） | 小 | パフォーマンス |
| F26 | [ ] | biome 既存エラー・警告の解消 | 小 | 品質 |
