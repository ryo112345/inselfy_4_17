...
**実施メモ（2026-07-16、3-4 完了 — Phase 3 これで全完了。Echo は go.mod から消えた）:**

- **ミドルウェアは4本とも `func(http.Handler) http.Handler` に移植**し、チェーン順は echo 時代と
  同一（Recover → RequestLogging → OpenAPI検証＋認可 → CORS → ルーター）。CORS は echomw の
  使用サブセット（credentialed 完全一致 allowlist・preflight は origin 不許可でも 204 終端）を
  自作で再現、`rs/cors` は入れない。validator の echo context ミラー（`c.Set(UserIDKey)` 等）は
  全 handler が context 版に移行済みのため削除。
- **ロガー enrich の上流伝播は mutable holder 方式**: echo は `c.SetRequest` で上書きできたが
  net/http は上流が古い context しか持たない。`logging.WithLogger` が holder を挿し、
  `logging.SetLogger` が中身を差し替える（admin 監査ログの `admin_auth`/`admin_id` が
  アクセスログに乗ることを実機確認済み）。
- **statusRecorder（アクセスログ/Recover 用の ResponseWriter ラッパー）は `Unwrap()` を実装**。
  coder/websocket は `rwUnwrapper` を辿って Hijacker を見つけるため、WS アップグレード
  （101）はラッパー越しに動く（実接続で確認済み）。
- **スペック外ルートも全て strictRouter（ServeMux）へ**: health 4本・WS 2本・Stripe webhook・
  静的配信（`GET /api/uploads/{path...}`、http.Dir + IsDir→404 で echo Static 相当。
  FileServer のディレクトリリスティングは出さない）。spec_drift_test は echo ルート表参照を
  やめ `sr.patterns` のみで検査。
- **未登録パスの 404/405 は echo の JSON ボディ（`{"message":"Not Found"}` 等）を維持**:
  ServeMux のフォールバックハンドラ（404/405/301 の判定と Allow/Location 計算を持つ）を
  probe に流して verdict とヘッダーだけ取り、ボディを JSON に差し替える。
  ⚠️ 初版で Location を落として `//` 連続スラッシュの正規化リダイレクトを壊しかけた
  （probe から Content-Type/Length 以外の全ヘッダーをコピーする形で解消）。
- **main.go は http.Server 直**。ReadHeaderTimeout 10s / IdleTimeout 120s のみ設定し、
  Read/WriteTimeout は付けない（/api/ws が長寿命接続を張るため全体タイムアウトは殺す）。
- depguard に `no-echo` ルールを追加（labstack/echo を import したら lint で落ちる）。
  echo 専用ヘルパー（jwt/company/any MW・context.go の echo 版・errors.go の
  badRequest/unauthorized/internalError）を削除、WS/Stripe controller は素の net/http に書き換え。
- **検証: 全11グループのスモークを移行前後で再実行、482 ケース 0 diffs**。
  スペック外ルート（health/WS ticket/webhook 署名エラー/uploads 実ファイル/404/405/CORS
  preflight・actual・不許可 origin）も前後比較して一致。意図的微差は
  405 の `Allow` 値（echo: OPTIONS,GET → ServeMux: GET,HEAD）と `X-Content-Type-Options:
  nosniff` の追加のみ。
- スモークで見えた 2 件の差分はどちらも**移行と無関係の既存非決定性**と特定し、
  diff_bodies.py の正規化で吸収: ① WV 結果の needs 配列（presenter が map を不安定ソート
  するため同点 displayScore のタイ順序と rank 割当がリクエストごとに揺れる。同一サーバーへの
  連続 curl で flip を実証）、② admin ユーザー一覧（seed 一括投入の created_at 同値タイが
  ORDER BY のタイブレーク無しで heap 順依存）。root cause 修正（needId タイブレーク等）は
  挙動変更になるため本フェーズではやらない（バックログ行き）。
