...
1点だけ計画からの逸脱: frontend の biome は **recommended リンタールールに既存858件の違反**
があったため、`biome.jsonc` で当面 linter を無効化し「フォーマット＋import整列」だけをゲートに
した（導入時に201ファイルを一括正規化済み）。ルールの段階有効化は別タスク。
