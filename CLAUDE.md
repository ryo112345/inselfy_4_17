...
「未生成のレポートを処理して」「レポートを生成して」等と依頼されたら、以下の手順で処理する。
AIレポートはバックエンドでの自動生成を行わず、Claude Codeがバッチ生成する。
管理画面: http://localhost:5173/admin/reports

**認証:** 職務経歴書ワークフローと同じく `X-Admin-Key` ヘッダーに環境変数 `ADMIN_API_KEY` の値を使用する。
...
```
レスポンスから以下を確認する:
- `needs[]`: 21項目のスコア（`needKey`, `mu`, `se`, `displayScore`, `rank`, `descriptionJa`）
- `consistency`: 整合性情報（`consistencyCoefficient`, `consistencyLevel`）

### 3. レポートを生成する

取得した診断結果を元に、以下のルールでレポートを生成する:

**統計的有意性の判定:**
- z = μ / SE を計算し、z > 1.64 なら「明らかに重視している」、z < -1.64 なら「明らかに重視していない」
- それ以外の項目は判定不能として一切言及しない

**レポートの文体:**
- 温かく寄り添うキャリアカウンセラーの語り口
- 「〜ですね」「〜ではないでしょうか」のように柔らかい表現
- 丁寧語は保つが馴れ馴れしくならない

**レポート構成（マークダウン形式）:**
1. **キャッチコピー**: 価値観に合った実在の著名人の名言を引用
2. **あなたの価値観プロファイル**: 重視/非重視グループから読み取れる特徴（200文字以内×2段落）
3. **適した職業・働き方**: 3つ提案、各200文字以内
4. **適さない可能性がある職業・働き方**: 各200文字以内
5. **キャリア形成のアドバイス**: 200文字以内

**注意事項:**
- 判定不能な項目を根拠にした分析は一切しない
- グループ内の項目同士に順位をつけない
- 具体的な統計データ・年収数値・求人倍率は提示しない
- ダッシュ記号（——、―、−、–、—）は使わない。補足は「、」や括弧で表現
- 水平線（---、***）は使わない。セクション間は見出し（###）のみ
- レポート本文のみ出力。タイトルや導入文は不要
