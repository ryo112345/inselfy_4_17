...
- **後方互換（expand-contract）縛り:** 本番 CD はデプロイ前に migrate が走り、ロールバック先の
...
APIのベースURLは `http://localhost:3000`（Next.js dev の /api プロキシ経由）、
またはバックエンド直（`http://localhost:8081`）を使用する。
管理画面: http://localhost:3000/admin/resumes

**認証:** admin APIには `X-Admin-Key` ヘッダーが必要。環境変数 `ADMIN_API_KEY` の値を使用する。
この値は管理画面（/admin/admins）で管理者ごとに発行される個人用APIトークン（`admin_...`形式）。
承認（approve）は内容確認の責任を管理者が持つため、Claude Codeからは実行せず、
管理者が管理画面のドラフト確認ページから行う。
...
```
確認してください: http://localhost:3000/admin/resumes/{resumeId}/draft
```
管理者がこのページで内容を確認・編集し、「承認」ボタンを押す。承認するとプロフィールに自動反映される（職歴・学歴は全置き換え、スキルは追加、headline/about 等は上書き）。
